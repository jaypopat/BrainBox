            ...
            if let Some(current_id) = current_node_id {
                if let Some(value) = self.db.get(&current_id).expect("Failed to get node") {
                    ...
                    if content_response.changed() {
                        // Update links and the database with the latest content
                        ...
                        // Refresh the node from the database to ensure all updates are reflected
                        let updated_value = self
                            .db
                            .get(&current_id)
                            .expect("Failed to get node")
                            .expect("Node not found");
                        node = serde_json::from_slice(&updated_value)
                    ...
                    // Update the node if the title or content was modified
                    if node_title != node.title || node_content != node.content {
